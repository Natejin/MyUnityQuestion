...
public:
	...
	inline RECT GetBoundingBox() {
		RECT rc = RectMakeCenter(
			(int)m_imageInfo->x, (int)m_imageInfo->y,
			(int)m_imageInfo->width, (int)(int)m_imageInfo->height);
		return rc;
	}
	...
	inline RECT GetFrameBoundingBox() {
		RECT rc = RectMakeCenter(
			(int)m_imageInfo->x, (int)m_imageInfo->y,
			(int)m_imageInfo->frameWidth, (int)m_imageInfo->frameHeight);
		return rc;
	}

	// 프레임 X get set
	inline int GetFrameX() { return (int)m_imageInfo->currentFrameX; }
	inline void SetFrameX(int frameX) { (int)m_imageInfo->currentFrameX = frameX; }
	// 프레임 Y get set
